...
export default function MenuPage() {
  ...
  // Seed translation memory from loaded data
  useEffect(() => {
    if (categories.length || products.length || modifierGroups.length || modOptions.length) {
      memorySeed([...categories, ...products, ...modifierGroups, ...modOptions] as Array<{
        name_es: string; name_en?: string; name_fr?: string; name_de?: string; name_it?: string;
      }>);
    }
  }, [categories, products, modifierGroups, modOptions]);
  ...
  /* ── Auto-translate on name_es blur ──────────────── */
  ...
  const autoTranslate = async (
    ...
    const t = await translateText(esText);
    if (t) {
      setter((prev: Record<string, unknown>) => ({
        ...prev,
        name_en: prev.name_en || t.en,
        name_fr: (prev.name_fr as string) || t.fr,
        name_de: (prev.name_de as string) || t.de,
        name_it: (prev.name_it as string) || t.it,
      }));
    }
  };
  ...
  const handleTranslateMenu = async () => {
    ...
    try {
      const res = await fetch("/api/ai/translate-menu", { method: "POST" });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Translation failed");
      setTranslateMsg(`${data.translated} ${t("menu.translations_saved")}`);
      Promise.all([loadCategories(), loadProducts(), loadModifierGroups(), loadModifierOptions()]);
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : "Error";
      setTranslateMsg(`Error: ${msg}`);
    } finally {
    ...
    <div style={{ padding: 24, flex: 1, minHeight: 0, overflowY: "auto" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          ...
          {/* Translate menu to all languages */}
          <button
            style={{
              ...btnSecondary,
              display: "flex",
          ...
          >
            <Globe size={16} />
            {translating ? t("menu.translating") : t("menu.translate_menu")}
          </button>
          {translateMsg && (
            <span style={{ fontSize: 12, color: translateMsg.startsWith("Error") ? "#f87171" : "#34d399" }}>
              {translateMsg}
            </span>
          )}
            ...
            {group.prods.map((prod) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    {prod.image_url ? (
                      <img src={encodeURI(prod.image_url)} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                    ) : (
                      "🍽️"
                    )}
                  </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div>
              ...
              <input
                ...
                onBlur={() => autoTranslate(catForm.name_es, setCatForm as never)}
              />
            </div>
              ...
              {(["en", "fr", "de", "it"] as const).map((l) => (
                <div key={l}>
                  ...
                  <input
                    ...
                    value={(catForm as Record<string, unknown>)[`name_${l}`] as string || ""}
                    onChange={(e) => setCatForm({ ...catForm, [`name_${l}`]: e.target.value })}
                    placeholder={`Auto-${l}`}
                  />
                </div>
              ))}
        ...
        <Overlay onClose={() => setProdModal(false)}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <div>
                <label style={labelStyle}>{t("menu.name_es")} *</label>
                <input style={inputStyle} value={prodForm.name_es} onChange={(e) => setProdForm({ ...prodForm, name_es: e.target.value })} onBlur={() => autoTranslate(prodForm.name_es, setProdForm as never)} />
              </div>
              <div>
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                ...
                {prodForm.image_url ? (
                  ...
                  >
                    <img
                      src={encodeURI(prodForm.image_url)}
                      alt=""
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div>
              ...
              <input
                ...
                onBlur={() => autoTranslate(modGroupForm.name_es, setModGroupForm as never)}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {(["en", "fr", "de", "it"] as const).map((l) => (
                <div key={l}>
                  ...
                  <input
                    style={{ ...inputStyle, fontSize: 13 }}
                    value={(modGroupForm as Record<string, unknown>)[`name_${l}`] as string || ""}
                    onChange={(e) => setModGroupForm({ ...modGroupForm, [`name_${l}`]: e.target.value })}
                    placeholder={`Auto-${l}`}
                  />
                </div>
              ))}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div>
              ...
              <input
                ...
                onBlur={() => autoTranslate(modOptForm.name_es, setModOptForm as never)}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {(["en", "fr", "de", "it"] as const).map((l) => (
                <div key={l}>
                  ...
                  <input
                    style={{ ...inputStyle, fontSize: 13 }}
                    value={(modOptForm as Record<string, unknown>)[`name_${l}`] as string || ""}
                    onChange={(e) => setModOptForm({ ...modOptForm, [`name_${l}`]: e.target.value })}
                    placeholder={`Auto-${l}`}
                  />
                </div>
              ))}
