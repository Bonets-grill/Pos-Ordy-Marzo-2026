...
interface Modifier {
  id: string; group_id: string; name_es: string; name_en: string; price_delta: number;
}
...
function localDesc(item: { description_es?: string | null; description_en?: string | null }, lang: Lang): string {
  if (lang === "es") return item.description_es || item.description_en || "";
  return item.description_en || item.description_es || "";
}
...
export default function QRMenuPage() {
  ...
  const loadMenu = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/public/menu?slug=${slug}&table=${tableNumber}`);
  ...
  const sendOrder = async () => {
    ...
    try {
      ...
      const orderItems = cart.map((c) => ({
        ...
        name: tenantName(c.menuItem, tenantLang), // TENANT language for KDS
        quantity: c.qty,
        ...
        modifiers_total: c.selectedModifiers.reduce((s, m) => s + m.price_delta, 0),
      ...
      const res = await fetch("/api/public/order", {
        ...
        body: JSON.stringify({
          tenantSlug: slug,
          tableNumber,
          customerLang: lang, // customer's language for notifications
