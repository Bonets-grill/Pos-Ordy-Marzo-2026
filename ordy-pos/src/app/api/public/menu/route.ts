...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data: items } = await supabase
      .from("menu_items")
      .select("id, category_id, name_es, name_en, name_fr, name_de, name_it, description_es, description_en, price, image_url, available, allergens, prep_time_minutes, sort_order")
      .eq("tenant_id", tenantId)
      .eq("active", true)
      .eq("available", true)
      .order("sort_order", { ascending: true })
